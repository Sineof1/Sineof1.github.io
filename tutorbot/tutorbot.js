...
$(document).ready(function(){$('.speech_bubble').offset({left : 250, top : $('.bot').offset().top - 175});});
...
function endTriangle(butID) {
  ...
  else if (curFocus === 'wrapUpQuestion' && errGN1 + errGN2 + errGN3 + errGN4 < 6) hintGroup4();
  else if (errGN1 + errGN2 + errGN3 + errGN4 >= 6) hintGroup5();
  }
}
function answerDraw() {
...
$('#slope1').bind('keyup', function(evt) {
   ...
   if (botClick % 2 === 0) {
     ...
     $('.speech_bubble').offset({top : $('.bot').offset().top - 175});
   }
...
$('#equation2').bind('keyup', function(evt) {
   ...
   botUI.message.removeAll();
   if (curFocus === 'wrapUpQuestion' && errGN1 + errGN2 + errGN3 + errGN4 < 6) hintGroup4();
   else if (errGN1 + errGN2 + errGN3 + errGN4 >= 6) hintGroup5();
...
$('.speech_bubble').offset({left : 250, top : $('.bot').offset().top - 175});
...
$(document).on('click', '.bot', function(evt) {
   ...
   if (curFocus === 'slope1' || curFocus === 'disclose1') {
      ...
      var styleElem = document.head.appendChild(document.createElement("style"));
      styleElem.innerHTML = ".speech_bubble:after {top:70.6%;}";
      $('.speech_bubble').offset({left : 250, top : $('.bot').offset().top - 210});
   }
   else {
      $('.speech_bubble').css({'height' : '375px'});
      var styleElem = document.head.appendChild(document.createElement("style"));
      styleElem.innerHTML = ".speech_bubble:after {top:66.67%;}";
      $('.speech_bubble').offset({left : 250, top : $('.bot').offset().top - 165});
   }
   var d = new Date();
   $('#botImage').attr('src', 'tutorbot_neutral.gif?' + d.getTime());
   ...
   setTimeout(function(){
      ...
      $('.speech_bubble').offset({left : 975, top : $('.bot').offset().top - 175});
      }
      else $('.speech_bubble').offset({left : 250, top : $('.bot').offset().top - 175});
      $('#equation2, #equation3').css('box-shadow', '0 0 3px #aaa');
   }, 100);
   botstart.pause();
   botstart.currentTime = 0;
  ...
  botUI.message.bot({type : 'html', content: 'Click on a point 🔵 to draw a <strong>slope triangle</strong>.<br /><br />Don\'t hold down the mouse button. Just click and move. 😃'})
               ...
               .then(function(res){
                  ...
                  botUI.message.bot({type : 'html', delay: 2000, loading: true, content: $('#hint0').html()})
                       ...
                       .then(function(){var d = new Date(); $('#botImage').attr('src', 'tutorbot_neutral.gif?' + d.getTime());});
                  }
  ...
  botUI.message.bot({type : 'html', content: 'Thanks for talking with me!<br /><br />So, you want to determine the <strong>slope</strong> of <strong>line g</strong>.  😃'})
               .then(function(){
                  ...
                  return botUI.action.button({action: [
                     ...
                     {cssClass : 'exampBut', text : 'I want to see some choices.', value : 'mltChHint'},
...
function hintGroup2() {
  botInx += 1;
  setTimeout(function(){$('.botui').prop('scrollTop', 0);}, 500);
  botUI.message.bot({type : 'html', content: 'Oh, good! Let\'s talk.<br /><br />Now you want to complete the <strong>equation</strong> y = ⬜x for <strong>line g</strong>.  😊'})
               .then(function(){
